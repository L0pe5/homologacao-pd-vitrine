...
function preencherBadges(badgesSrcs) {
...
// Preenche os links de Aplicação e Repositório
async function preencherLinks(projetoId, webUrl) {
    const linksContainer = document.querySelector('.projects-links-container');
    if (!linksContainer) return;

    linksContainer.innerHTML = '';

    // Busca o link de deploy (badge "deploy")
    ...
    // Link do "Repositório"
    linksContainer.innerHTML += `<a href="${webUrl}" class="btn btn-project" target="_blank" rel="noopener noreferrer">Repositório</a>`;
}

// Preenche a lista de Colaboradores e adiciona o listener de clique
...
// --- Funções da Galeria (já estão bem estruturadas) ---
...
// configura os listeners do modal de galeria
...
// Inicializa a página do projeto
document.addEventListener('DOMContentLoaded', async function () {
    ...
    try {
        // busca todos os dados da API primeiro
        ...
        // define o estado global das imagens
        projectImages = await Promise.all(imagensUrls.map(url => carregarImagemPrivada(url)));

        // chama as funções "helper" para popular o DOM
        preencherInfoPrincipal(projeto, readmeContent);
